...
bool hay_accion_que_toque(accion_t* accion, char* nombre) {
    ...
    while (accion)
	{
		carta_t *dest = accion->destino;
		int check = strcmp(dest->nombre, nombre);
		if (check == 0)
		{
			res = true;
			break;
...
void invocar_acciones(accion_t* accion, tablero_t* tablero) {
	while (accion)
	{
		...
		if (destino->en_juego)
		{
			accion->invocar(tablero, destino);
			if (destino->vida == 0)
			{	
...
void contar_cartas(tablero_t* tablero, uint32_t* cant_rojas, uint32_t* cant_azules) {
	*cant_rojas = 0;
	*cant_azules = 0;
	int azul = tablero->mano_jugador_azul->jugador;
	int rojo = tablero->mano_jugador_rojo->jugador;
	for (int i = 0; i < ALTO_CAMPO; i++)
	{
		for (int j = 0; j < ANCHO_CAMPO; j++)
		{
			carta_t *posicion = tablero->campo[i][j];
			if (posicion)
			{
				bool ocupado = posicion->en_juego;
				if (ocupado)
				{
					int dueño = posicion->jugador;
					if (dueño == azul)
					{
						(*cant_azules)++;
					}
